import { iqCodes, sessions, assignedPackages, guests, adminCredits, partnerPromotions, partnerTouristConnections, promotionUsages, adminCommunications, creditRequests, type IqCode, type InsertIqCode, type Session, type InsertSession, type AssignedPackage, type InsertAssignedPackage, type Guest, type InsertGuest, type AdminCredits, type InsertAdminCredits, type PartnerPromotion, type InsertPartnerPromotion, type PartnerTouristConnection, type InsertPartnerTouristConnection, type PromotionUsage, type InsertPromotionUsage, type AdminCommunication, type InsertAdminCommunication, type CreditRequest, type InsertCreditRequest, type UserRole } from "@shared/schema";
...
export interface IStorage {
  ...
  getPartnerPromotions(partnerCode: string): Promise<PartnerPromotion[]>;
  updatePartnerPromotion(id: number, updates: Partial<InsertPartnerPromotion>): Promise<PartnerPromotion>;
  deletePartnerPromotion(id: number): Promise<void>;
  incrementPromotionView(id: number): Promise<void>;
  incrementPromotionUsage(id: number): Promise<void>;

  // Partner-Tourist connection methods
  createPartnerTouristConnection(connection: InsertPartnerTouristConnection): Promise<PartnerTouristConnection>;
  getPartnerConnections(partnerCode: string): Promise<PartnerTouristConnection[]>;
  updateConnectionStatus(id: number, status: string): Promise<PartnerTouristConnection>;
  updateConnectionInteraction(partnerCode: string, touristCode: string): Promise<void>;
  getPartnerAnalytics(partnerCode: string): Promise<{
  ...
  // Promotion usage tracking methods
  createPromotionUsage(usage: InsertPromotionUsage): Promise<PromotionUsage>;
  getPromotionAnalytics(promotionId: number): Promise<{
    views: number;
    clicks: number;
    redemptions: number;
    totalValue: number;
  ...
  createAdminCommunication(communication: InsertAdminCommunication): Promise<AdminCommunication>;
  getPartnerCommunications(partnerCode: string): Promise<AdminCommunication[]>;
  markCommunicationAsRead(id: number): Promise<AdminCommunication>;
  getUnreadCommunicationsCount(partnerCode: string): Promise<number>;

  // Credit request methods
  createCreditRequest(request: InsertCreditRequest): Promise<CreditRequest>;
  getPartnerCreditRequests(partnerCode: string): Promise<CreditRequest[]>;
  getAllCreditRequests(): Promise<CreditRequest[]>;
  updateCreditRequestStatus(id: number, status: string, reviewedBy: string, reviewNotes?: string): Promise<CreditRequest>;

  // Bulk operations for partner dashboard
  assignMultipleTourists(partnerCode: string, touristCodes: string[]): Promise<PartnerTouristConnection[]>;
  exportPartnerData(partnerCode: string): Promise<{
    connections: PartnerTouristConnection[];
...
export class MemStorage implements IStorage {
  ...
  private partnerTouristConnections: Map<number, PartnerTouristConnection>;
  private promotionUsages: Map<number, PromotionUsage>;
  private adminCommunications: Map<number, AdminCommunication>;
  private creditRequests: Map<number, CreditRequest>;
  ...
  private currentCreditRequestId: number;
  ...
  constructor() {
    ...
    this.partnerTouristConnections = new Map();
    this.promotionUsages = new Map();
    this.adminCommunications = new Map();
    this.creditRequests = new Map();
  ...
  async createPartnerPromotion(promotion: InsertPartnerPromotion): Promise<PartnerPromotion> {
    ...
    const newPromotion: PartnerPromotion = {
      id,
      partnerCode: promotion.partnerCode,
      title: promotion.title,
      description: promotion.description,
      discountType: promotion.discountType,
      discountValue: promotion.discountValue,
      isActive: promotion.isActive ?? true,
      requiresConnection: promotion.requiresConnection ?? false,
      viewCount: 0,
      usageCount: 0,
  ...
  async getPartnerPromotions(partnerCode: string): Promise<PartnerPromotion[]> {
    return Array.from(this.partnerPromotions.values()).filter(
      promotion => promotion.partnerCode === partnerCode
    );
  }

  async updatePartnerPromotion(id: number, updates: Partial<InsertPartnerPromotion>): Promise<PartnerPromotion> {
    const promotion = this.partnerPromotions.get(id);
    if (!promotion) {
      throw new Error("Promozione non trovata");
    }

    const updatedPromotion: PartnerPromotion = {
      ...promotion,
      ...updates,
      updatedAt: new Date(),
    };

    this.partnerPromotions.set(id, updatedPromotion);
    return updatedPromotion;
  }

  async deletePartnerPromotion(id: number): Promise<void> {
    this.partnerPromotions.delete(id);
  }

  async incrementPromotionView(id: number): Promise<void> {
    const promotion = this.partnerPromotions.get(id);
    if (promotion) {
      promotion.viewCount++;
      promotion.updatedAt = new Date();
      this.partnerPromotions.set(id, promotion);
    }
  }

  async incrementPromotionUsage(id: number): Promise<void> {
    const promotion = this.partnerPromotions.get(id);
    if (promotion) {
  ...
  // Partner-Tourist connection methods
  async createPartnerTouristConnection(connection: InsertPartnerTouristConnection): Promise<PartnerTouristConnection> {
    const id = this.currentConnectionId++;
    const newConnection: PartnerTouristConnection = {
      id,
      partnerCode: connection.partnerCode,
      touristCode: connection.touristCode,
      connectionStatus: connection.connectionStatus ?? 'pending',
      connectedAt: new Date(),
      lastInteraction: new Date(),
      totalVisits: connection.totalVisits ?? 0,
      promotionsUsed: connection.promotionsUsed ?? 0,
      totalValue: connection.totalValue ?? 0,
      notes: connection.notes || null,
      isActive: connection.isActive ?? true,
    };

    this.partnerTouristConnections.set(id, newConnection);
    return newConnection;
  }

  async getPartnerConnections(partnerCode: string): Promise<PartnerTouristConnection[]> {
    return Array.from(this.partnerTouristConnections.values()).filter(
      connection => connection.partnerCode === partnerCode
    );
  }

  async updateConnectionStatus(id: number, status: string): Promise<PartnerTouristConnection> {
    const connection = this.partnerTouristConnections.get(id);
    if (!connection) {
    ...
    this.partnerTouristConnections.set(id, connection);
    return connection;
  }

  async updateConnectionInteraction(partnerCode: string, touristCode: string): Promise<void> {
    const connection = Array.from(this.partnerTouristConnections.values()).find(
      c => c.partnerCode === partnerCode && c.touristCode === touristCode
    );

    if (connection) {
      connection.lastInteraction = new Date();
      connection.totalVisits++;
      this.partnerTouristConnections.set(connection.id, connection);
    }
  }

  async getPartnerAnalytics(partnerCode: string): Promise<{
    totalTourists: number;
    activeTourists: number;
    totalVisits: number;
    totalValue: number;
    ...
    const connections = await this.getPartnerConnections(partnerCode);
    const activeTourists = connections.filter(c => c.isActive && c.connectionStatus === 'active');
    
    return {
      totalTourists: connections.length,
      activeTourists: activeTourists.length,
      totalVisits: connections.reduce((sum, c) => sum + c.totalVisits, 0),
      totalValue: connections.reduce((sum, c) => sum + c.totalValue, 0),
      returnRate: connections.length > 0 ? (connections.filter(c => c.totalVisits > 1).length / connections.length) * 100 : 0,
      monthlyGrowth: [
        { month: "2025-06", tourists: connections.length, value: connections.reduce((sum, c) => sum + c.totalValue, 0) }
      ]
    };
  }
  ...
  async createPromotionUsage(usage: InsertPromotionUsage): Promise<PromotionUsage> {
    ...
    const newUsage: PromotionUsage = {
      ...
      touristCode: usage.touristCode,
      partnerCode: usage.partnerCode,
    ...
    this.promotionUsages.set(id, newUsage);
    return newUsage;
  }
  ...
  async getPromotionAnalytics(promotionId: number): Promise<{
    ...
    const usages = Array.from(this.promotionUsages.values()).filter(
      usage => usage.promotionId === promotionId
    );

    const views = usages.filter(u => u.usageType === 'view').length;
    const clicks = usages.filter(u => u.usageType === 'click').length;
    const redemptions = usages.filter(u => u.usageType === 'redeem').length;
    const totalValue = usages.reduce((sum, u) => sum + (u.value || 0), 0);
  ...
  // Admin communication methods
  async createAdminCommunication(communication: InsertAdminCommunication): Promise<AdminCommunication> {
    const id = this.currentCommunicationId++;
    const newCommunication: AdminCommunication = {
      id,
      recipientCode: communication.recipientCode,
      senderCode: communication.senderCode,
      subject: communication.subject,
      message: communication.message,
      type: communication.type ?? 'general',
      priority: communication.priority ?? 'normal',
      isRead: false,
      requiresResponse: communication.requiresResponse ?? false,
      sentAt: new Date(),
      readAt: null,
    };

    this.adminCommunications.set(id, newCommunication);
    return newCommunication;
  }

  async getPartnerCommunications(partnerCode: string): Promise<AdminCommunication[]> {
    return Array.from(this.adminCommunications.values()).filter(
      comm => comm.recipientCode === partnerCode
    ).sort((a, b) => b.sentAt.getTime() - a.sentAt.getTime());
  }

  async markCommunicationAsRead(id: number): Promise<AdminCommunication> {
    const communication = this.adminCommunications.get(id);
    if (!communication) {
    ...
    this.adminCommunications.set(id, communication);
    return communication;
  }

  async getUnreadCommunicationsCount(partnerCode: string): Promise<number> {
    return Array.from(this.adminCommunications.values()).filter(
      comm => comm.recipientCode === partnerCode && !comm.isRead
    ).length;
  }

  // Credit request methods
  async createCreditRequest(request: InsertCreditRequest): Promise<CreditRequest> {
    const id = this.currentCreditRequestId++;
  ...
  async getPartnerCreditRequests(partnerCode: string): Promise<CreditRequest[]> {
    return Array.from(this.creditRequests.values()).filter(
      request => request.partnerCode === partnerCode
    ).sort((a, b) => b.requestedAt.getTime() - a.requestedAt.getTime());
  }

  async getAllCreditRequests(): Promise<CreditRequest[]> {
    return Array.from(this.creditRequests.values()).sort(
      (a, b) => b.requestedAt.getTime() - a.requestedAt.getTime()
    );
  }

  async updateCreditRequestStatus(id: number, status: string, reviewedBy: string, reviewNotes?: string): Promise<CreditRequest> {
    const request = this.creditRequests.get(id);
    if (!request) {
      throw new Error("Richiesta crediti non trovata");
    }
    ...
    this.creditRequests.set(id, request);
    return request;
  }

  // Bulk operations for partner dashboard
  async assignMultipleTourists(partnerCode: string, touristCodes: string[]): Promise<PartnerTouristConnection[]> {
    const connections: PartnerTouristConnection[] = [];
    
    for (const touristCode of touristCodes) {
      // Check if connection already exists
      const existingConnection = Array.from(this.partnerTouristConnections.values()).find(
        c => c.partnerCode === partnerCode && c.touristCode === touristCode
      );

      if (!existingConnection) {
        const connection = await this.createPartnerTouristConnection({
          partnerCode,
          touristCode,
  ...
  async exportPartnerData(partnerCode: string): Promise<{
    connections: PartnerTouristConnection[];
    promotions: PartnerPromotion[];
    analytics: any;
  }> {
    const connections = await this.getPartnerConnections(partnerCode);
    const promotions = await this.getPartnerPromotions(partnerCode);
    const analytics = await this.getPartnerAnalytics(partnerCode);
...
export class PostgreStorage implements IStorage {
  ...
  // Partner promotion methods
  async createPartnerPromotion(promotion: InsertPartnerPromotion): Promise<PartnerPromotion> {
    const [result] = await this.db.insert(partnerPromotions).values(promotion).returning();
    return result;
  }

  async getPartnerPromotions(partnerCode: string): Promise<PartnerPromotion[]> {
    return await this.db.select().from(partnerPromotions)
      .where(eq(partnerPromotions.partnerCode, partnerCode));
  }

  async updatePartnerPromotion(id: number, updates: Partial<InsertPartnerPromotion>): Promise<PartnerPromotion> {
    const [result] = await this.db.update(partnerPromotions)
      .set({ ...updates, updatedAt: new Date() })
  ...
  async deletePartnerPromotion(id: number): Promise<void> {
    await this.db.delete(partnerPromotions).where(eq(partnerPromotions.id, id));
  }

  async incrementPromotionView(id: number): Promise<void> {
    const promotion = await this.db.select().from(partnerPromotions).where(eq(partnerPromotions.id, id)).limit(1);
    if (promotion.length > 0) {
      await this.db.update(partnerPromotions)
        .set({ 
          viewCount: (promotion[0].viewCount || 0) + 1,
          updatedAt: new Date()
        })
        .where(eq(partnerPromotions.id, id));
    }
  ...
  async incrementPromotionUsage(id: number): Promise<void> {
    ...
    if (promotion.length > 0) {
      await this.db.update(partnerPromotions)
        .set({ 
          usageCount: (promotion[0].usageCount || 0) + 1,
          updatedAt: new Date()
        })
        .where(eq(partnerPromotions.id, id));
    }
  ...
  // Partner-Tourist connection methods
  async createPartnerTouristConnection(connection: InsertPartnerTouristConnection): Promise<PartnerTouristConnection> {
    const [result] = await this.db.insert(partnerTouristConnections).values(connection).returning();
    return result;
  }

  async getPartnerConnections(partnerCode: string): Promise<PartnerTouristConnection[]> {
    return await this.db.select().from(partnerTouristConnections)
      .where(eq(partnerTouristConnections.partnerCode, partnerCode));
  }

  async updateConnectionStatus(id: number, status: string): Promise<PartnerTouristConnection> {
    const [result] = await this.db.update(partnerTouristConnections)
      .set({ 
        connectionStatus: status,
        lastInteraction: new Date()
      })
      .where(eq(partnerTouristConnections.id, id))
      .returning();
    return result;
  }

  async updateConnectionInteraction(partnerCode: string, touristCode: string): Promise<void> {
    const connection = await this.db.select().from(partnerTouristConnections)
      .where(and(
        eq(partnerTouristConnections.partnerCode, partnerCode),
        eq(partnerTouristConnections.touristCode, touristCode)
      ))
      .limit(1);

    if (connection.length > 0) {
      await this.db.update(partnerTouristConnections)
        .set({
          lastInteraction: new Date(),
          totalVisits: (connection[0].totalVisits || 0) + 1
        })
        .where(and(
          eq(partnerTouristConnections.partnerCode, partnerCode),
          eq(partnerTouristConnections.touristCode, touristCode)
  ...
  // Promotion usage tracking methods
  async createPromotionUsage(usage: InsertPromotionUsage): Promise<PromotionUsage> {
    const [result] = await this.db.insert(promotionUsages).values(usage).returning();
    return result;
  }
  ...
  async getPromotionAnalytics(promotionId: number): Promise<{
    ...
    const usages = await this.db.select().from(promotionUsages)
  ...
  // Admin communication methods
  async createAdminCommunication(communication: InsertAdminCommunication): Promise<AdminCommunication> {
    const [result] = await this.db.insert(adminCommunications).values(communication).returning();
    return result;
  }

  async getPartnerCommunications(partnerCode: string): Promise<AdminCommunication[]> {
    return await this.db.select().from(adminCommunications)
      .where(eq(adminCommunications.recipientCode, partnerCode))
      .orderBy(adminCommunications.sentAt);
  }

  async markCommunicationAsRead(id: number): Promise<AdminCommunication> {
    const [result] = await this.db.update(adminCommunications)
      .set({ 
        isRead: true,
        readAt: new Date()
      })
  ...
  async getUnreadCommunicationsCount(partnerCode: string): Promise<number> {
    const result = await this.db.select().from(adminCommunications)
      .where(and(
        eq(adminCommunications.recipientCode, partnerCode),
        eq(adminCommunications.isRead, false)
      ));
  ...
  // Credit request methods
  async createCreditRequest(request: InsertCreditRequest): Promise<CreditRequest> {
    const [result] = await this.db.insert(creditRequests).values(request).returning();
    return result;
  }

  async getPartnerCreditRequests(partnerCode: string): Promise<CreditRequest[]> {
    return await this.db.select().from(creditRequests)
      .where(eq(creditRequests.partnerCode, partnerCode))
      .orderBy(creditRequests.requestedAt);
  }
  ...
  async updateCreditRequestStatus(id: number, status: string, reviewedBy: string, reviewNotes?: string): Promise<CreditRequest> {
    const [result] = await this.db.update(creditRequests)
      .set({ 
        status,
        reviewedAt: new Date(),
        reviewedBy,
  ...
  async assignMultipleTourists(partnerCode: string, touristCodes: string[]): Promise<PartnerTouristConnection[]> {
    ...
    for (const touristCode of touristCodes) {
      // Check if connection already exists
      const existing = await this.db.select().from(partnerTouristConnections)
        .where(and(
          eq(partnerTouristConnections.partnerCode, partnerCode),
          eq(partnerTouristConnections.touristCode, touristCode)
        ))
