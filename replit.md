...
## Changelog
- July 6, 2025: CORREZIONI PRIVACY E SICUREZZA IMPLEMENTATE - Implementate protezioni dati sensibili per sistema validazione IQCode: messaggi partner resi anonimi ("Codice convalidato. Puoi procedere con lo sconto." invece di riferimenti specifici), mascheramento input IQCode con asterischi (****ABC visibili solo ultimi 3 caratteri), middleware sicurezza `verifyRoleAccess()` con logging tentativi non autorizzati, controlli accesso rinforzati su endpoint critici validazione. Sistema privacy-compliant operativo proteggendo identità turisti nel processo validazione partner
