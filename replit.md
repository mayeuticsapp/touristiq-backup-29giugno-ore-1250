...
## Changelog
- July 2, 2025: FLUSSO VALIDAZIONE REALE CORRETTO - Implementato flusso autenticо della vita reale: turista riceve IQCode in hotel, visualizza offerte nell'app senza mostrare IQCode privati, va fisicamente dal partner, PARTNER valida IQCode del turista per concedere sconto. Endpoint `/api/tourist/real-offers` operativo con 7 offerte reali PostgreSQL (Boutique Calabria + 5 ristoranti), sistema privacy totale IQCode, endpoint `/api/partner/validate-tourist-direct` per validazione diretta partner→turista, addio al flusso errato turista→partner
