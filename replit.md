...
## Changelog
- July 6, 2025: SISTEMA CUSTODE DEL CODICE ESTESO A STRUTTURE E PARTNER - Implementazione completa sistema "Custode del Codice" anche per dashboard strutture ricettive e partner business: componente riutilizzabile `CustodeCodiceDashboard` con stesso design e funzionalità del sistema turisti, posizionamento strategico sotto sezioni IQCode principali, tooltip informativi personalizzati per tipo ruolo (structure/partner), integrazione endpoints esistenti `/api/activate-custode` e `/api/check-custode-status`, supporto universale per tutti i ruoli TouristIQ (tourist/structure/partner), sistema anonimo completo senza email/telefono per tutte le tipologie utente
