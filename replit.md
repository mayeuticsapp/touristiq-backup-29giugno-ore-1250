...
## Changelog
- July 6, 2025: PRIVACY COMPLETA RAGGIUNTA - NASCONDIMENTO TOTALE CODICI IQ TURISTI - Implementato sistema privacy massimo: codici IQ turisti completamente nascosti dalla vista partner (sostituiti con "Richiesta di Validazione"), API backend sanitizzata rimuovendo campo `touristIqCode` dalle risposte `/api/iqcode/validation-status`, messaggi partner resi anonimi, mascheramento input IQCode con asterischi (****ABC), middleware sicurezza `verifyRoleAccess()` con logging, controlli accesso rinforzati. Sistema privacy-compliant operativo conformemente a principio guida "Il turista è protetto in ogni passaggio. Nessun partner può accedere alla sua identità se non dopo approvazione"
