...
## Changelog
- July 2, 2025: FLUSSO VALIDAZIONE CORRETTO SEMPLIFICATO - Implementato flusso corretto: Partner inserisce IQCode turista → Turista riceve richiesta nel pannello → Turista accetta/rifiuta → Partner riceve SOLO messaggio "Accettato" → Partner clicca "Utilizza IQCode" per scalare utilizzi. Eliminato doppio decremento bug, utilizzi scalano SOLO quando partner utilizza effettivamente il codice. Sistema privacy totale, endpoint `/api/tourist/real-offers` operativo con 7 offerte reali PostgreSQL
