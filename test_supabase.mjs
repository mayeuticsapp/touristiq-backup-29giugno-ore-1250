...
async function testConnection() {
  try {
    ...
    const adminCheck = await client.query("SELECT admin_code, credits_remaining FROM admin_credits WHERE admin_code = 'TIQ-IT-ADMIN'");
    if (adminCheck.rows.length > 0) {
      console.log('👑 Admin trovato:', adminCheck.rows[0].admin_code, 'con', adminCheck.rows[0].credits_remaining, 'crediti');
    }
